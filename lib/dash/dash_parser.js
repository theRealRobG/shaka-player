...
shaka.dash.DashParser = class {
  ...
  parseAdaptationSet_(context, elem) {
    ...
    for (const prop of essentialProperties) {
      ...
      } else if (schemeId == 'urn:mpeg:mpegB:cicp:TransferCharacteristics') {
        // CICP check for TransferCharacteristics is done when parsing
        // representation (for HDR indication).
      } else {
  ...
  parseRepresentation_(context, contentProtection, kind, language, label,
    ...
    const properties = context.adaptationSet.essentialProperties
        .concat(context.adaptationSet.supplementalProperties);

    // (DASH-IF IOP v4.3 10.3.3.)
    const hevcHDR = 'http://dashif.org/guidelines/dash-if-uhd#hevc-hdr-pq10';
    // (DASH-IF IOP v4.3 6.2.5.1.)
    const hdrScheme = 'urn:mpeg:mpegB:cicp:TransferCharacteristics';
    if (profiles.includes(hevcHDR) && (codecs.includes('hvc1.2.4.L153.B0') ||
        codecs.includes('hev1.2.4.L153.B0'))) {
      hdr = 'PQ';
    } else if (properties.length) {
      const hdrDescriptor = properties
          .find((property) => property.schemeIdUri === hdrScheme);
      if (hdrDescriptor) {
        // (ISO/IEC 23001-8)
        switch (parseInt(hdrDescriptor.value, 10)) {
          case 1:
          case 6:
          case 13:
          case 14:
  ...
  createFrame_(elem, parent, baseUris) {
    ...
    const essentialProperties = XmlUtils
        .findChildren((elem), 'EssentialProperty')
        .map((elem) => this.parseDescriptor_(elem))
        .filter((descriptor) => descriptor != null);
    const supplementalProperties = XmlUtils
        .findChildren((elem), 'SupplementalProperty')
        .map((elem) => this.parseDescriptor_(elem))
        .filter((descriptor) => descriptor != null);
  ...
  /**
   * @param {!Element} descriptorElement The descriptor element.
   * @return {?shaka.dash.DashParser.DescriptorType} The parsed descriptor, or
   *   null if `descriptorElement` has an invalid format.
   * @private
   */
  parseDescriptor_(descriptorElement) {
    const elementName = descriptorElement.nodeName;
    const schemeIdUri = descriptorElement.getAttribute('schemeIdUri');
    if (!elementName || !schemeIdUri) {
      return null;
    }
    const value = descriptorElement.getAttribute('value');
    const id = descriptorElement.getAttribute('id');
    return {elementName, schemeIdUri, value, id};
  }
 ...
 * contain a `@schemeIdUri` attribute that provides a URI to identify the
 * scheme and an optional attribute `@value` and an optional attribute `@id`.
 * The semantics of the element are specific to the scheme employed. The URI
 * identifying the scheme may be a URN or a URL.
 *
 * The MPD does not provide any specific information on how to use these
 * elements. It is up to the application that employs DASH formats to
 ...
 *
 * Two elements of type `DescriptorType` are equivalent, if the element name,
 * the value of the `@schemeIdUri` and the value of the `@value` attribute are
 * equivalent. If the `@schemeIdUri` is a URN, then equivalence shall refer to
 * lexical equivalence as defined in IETF RFC 8141:2017, Clause 3. If the
 * `@schemeIdUri` is a URL, then equivalence shall refer to equality on a
 * character-for-character basis as defined in IETF RFC 3986:2005, subclause
 * 6.2.1. If the `@value` attribute is not present, equivalence is determined
 * by the equivalence for `@schemeIdUri` only. Attributes and element in
 * extension namespaces are not used for determining equivalence. The `@id`
 * attribute may be ignored for equivalence determination.
 *
 * @property {string} elementName
 *   The element name for the descriptor.
 * @property {string} schemeIdUri
 *   Specifies a URI to identify the scheme. The semantics of this element are
 *   specific to the scheme specified by this attribute. The `@schemeIdUri` may
 *   be a URN or URL. When a URL is used, it should also contain a month-date
 *   in the form mmyyyy; the assignment of the URL must have been authorized by
 *   the owner of the domain name in that URL on or very close to that date, to
 *   avoid problems when domain names change ownership.
 * @property {(string|undefined)} value
 *   Specifies the value for the descriptor element. The value space and
 *   semantics must be defined by the owners of the scheme identified in the
 *   `@schemeIdUri` attribute.
 * @property {(string|undefined)} id
 *   Specifies an identifier for the descriptor. Descriptors with identical
 *   values for this attribute shall be synonymous, i.e. the processing of one
 *   of the descriptors with an identical value is sufficient.
 */
shaka.dash.DashParser.DescriptorType;
 ...
 *   essentialProperties: !Array.<shaka.dash.DashParser.DescriptorType>,
 *   supplementalProperties: !Array.<shaka.dash.DashParser.DescriptorType>
 * }}
 *
 * @description
 ...
 * @property {!Array.<shaka.dash.DashParser.DescriptorType>} essentialProperties
 *   The essential properties defined for the element.
 * @property {!Array.<shaka.dash.DashParser.DescriptorType>} supplementalProperties
 *   The supplemental properties defined for the element.
 */
